...
#define LPS_1T1R_ARG
	...
	info.dev2hst_gpio = WAKEUP_GPIO_IDX;
#ifdef CONFIG_RTW_ONE_PIN_GPIO
	info.dev2hst_gpio_mode = RTW_AX_SW_IO_MODE_INPUT;
	status = rtw_phl_cfg_wow_set_sw_gpio_mode(phl, &info);
#else
	#ifdef CONFIG_WAKEUP_GPIO_INPUT_MODE
	info.dev2hst_gpio_mode = RTW_AX_SW_IO_MODE_OUTPUT_OD;
	#else
	info.dev2hst_gpio_mode = RTW_AX_SW_IO_MODE_OUTPUT_PP;
	#endif /*CONFIG_WAKEUP_GPIO_INPUT_MODE*/
	/* switch GPIO to open-drain or push-pull */
	status = rtw_phl_cfg_wow_set_sw_gpio_mode(phl, &info);
	info.dev2hst_high = pwrctrlpriv->is_high_active == 0 ? 1 : 0;
	status = rtw_phl_cfg_wow_sw_gpio_ctrl(phl, &info);
	RTW_INFO("%s: set GPIO_%d %d as default. status=%d\n",
		 __func__, WAKEUP_GPIO_IDX, info.dev2hst_high, status);
