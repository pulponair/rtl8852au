			      ...
			      size_t count, loff_t *pos, void *data)
{
	...
	wowcam_info = (struct rtw_wowcam_upd_info *)rtw_zmalloc(sizeof(struct rtw_wowcam_upd_info));
	if (!wowcam_info) {
		RTW_ERR("%s: allocate wowcam_info fail\n", __func__);
		return -EFAULT;
	}
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if (ret) {
			...
			status = rtw_phl_add_wow_ptrn_info(phl, wowcam_info, &index);
		} else
			return -EFAULT;
				  ...
				  size_t count, loff_t *pos, void *data)
{
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		info.dev2hst_high = pwrpriv->is_high_active = is_high_active == 0 ? 1 : 0;
