...
static u8 rtw_tsf_sync_done_cmd_hdl(_adapter *adapter)
{
	struct dvobj_priv *d;
	void *phl;
	...
	status = rtw_phl_mr_state_upt(phl, adapter->phl_role);
	if (RTW_PHL_STATUS_SUCCESS != status) {
		RTW_ERR("%s fail(status=%d)", __func__, status);
		ret = _FAIL;
	}
...
u8 rtw_tsf_sync_done_cmd(_adapter *adapter, u8 enqueue)
{
	...
	parm->ec_id = TSF_SYNC_DONE_WK_CID;
	parm->type = 0;
	parm->size = 0;
	parm->pbuf = NULL;
	...
	if (!enqueue) {
		/* no need to enqueue, do the cmd hdl directly and free cmd parameter */
		if (_SUCCESS != rtw_tsf_sync_done_cmd_hdl(adapter))
			res = _FAIL;
		rtw_mfree((u8 *)parm, sizeof(*parm));
	} else {
		/* need enqueue, prepare cmd_obj and enqueue */
...
u8 rtw_drvextra_cmd_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	case TBTX_CONTROL_TX_WK_CID:
		...
		break;
#endif
	case TSF_SYNC_DONE_WK_CID:
		rtw_tsf_sync_done_cmd_hdl(padapter);
		break;
#ifdef ROKU_PRIVATE 
